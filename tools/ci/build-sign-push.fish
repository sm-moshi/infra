...
#   -n, --name IMAGE_NAME   Image name (required)
...
#
# Examples:
#   ./build-sign-push.fish -n myapp --tag v1.0.5 --platform linux/arm64,linux/amd64
#   ./build-sign-push.fish -n myapp -c /path/to/dockerfile/dir --push
#   TAG=v1.0.5 IMAGE_NAME=myapp ./build-sign-push.fish

# --- Parse Arguments ---
argparse h/help push latest no-sbom 't/tag=' 'p/platform=' 'n/name=' 'c/context=' 'project=' 'registry=' -- $argv
or begin
...
if set -q _flag_help
    ...
    echo "  -n, --name IMAGE_NAME   Image name (required)"
...
if set -q _flag_name; and test -n "$_flag_name"
    ...
    echo "❌ --name/-n is required"
    exit 1
end

if set -q _flag_context; and test -n "$_flag_context"
    set CONTEXT $_flag_context
else if set -q CONTEXT; and test -n "$CONTEXT"
    set CONTEXT $CONTEXT
else
    set CONTEXT .
end

if not test -d "$CONTEXT"
    echo "❌ Context path does not exist: $CONTEXT"
    exit 1
end

if not test -f "$CONTEXT/Dockerfile"
    echo "❌ No Dockerfile found in: $CONTEXT"
    exit 1
end
